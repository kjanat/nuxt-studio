...
export interface ModuleOptions {
  ...
  git?: {
     ...
     * even though the signature is technically valid.
     *
     * Only applies to GitHub. GitLab is out of scope — GitLab requires embedding
     * `gpgsig` in the raw git object, which cannot be done through its REST API.
     *
    ...
    signing?: {
      ...
      enabled?: boolean
      /**
       * ASCII-armored PGP private key used to sign commits.
       * @default process.env.STUDIO_SIGNING_PRIVATE_KEY
       */
      privateKey?: string
      /**
       * Passphrase protecting the private key (if any).
       * @default process.env.STUDIO_SIGNING_KEY_PASSPHRASE
       */
      passphrase?: string
    }
  }
  ...
  async setup(options, nuxt) {
    ...
    nuxt.options.runtimeConfig.public.studio = {
      ...
      git: {
        ...
        signing: { enabled: Boolean(options.git?.signing?.enabled) },
      },
    ...
    nuxt.options.runtimeConfig.studio = {
      ...
      git: {
        signing: {
          enabled: Boolean(options.git?.signing?.enabled),
          privateKey: options.git?.signing?.privateKey ?? process.env.STUDIO_SIGNING_PRIVATE_KEY ?? '',
          passphrase: options.git?.signing?.passphrase ?? process.env.STUDIO_SIGNING_KEY_PASSPHRASE ?? '',
        },
      },
    }
    ...
    addServerHandler({
      method: 'post',
      route: '/__nuxt_studio/git/sign-commit',
      handler: runtime('./server/routes/git/sign-commit.post'),
    })
  },
})
