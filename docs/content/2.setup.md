...
| **Cloudflare Workers / Pages**\* | from `.git/config`    | from `.git/config`       | from `.git/config`       | `WORKERS_CI_BRANCH` / `CF_PAGES_BRANCH` |

\* See [Git Providers › Automatic Detection](/git-providers#automatic-detection) for the full Cloudflare resolution rules.
