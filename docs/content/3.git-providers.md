...
## Commit Signing (GitHub only)

Studio can produce **PGP-signed commits** that show as "Verified" on GitHub. This is optional and disabled by default.

::note
Commit signing is only available for **GitHub** repositories. GitLab requires embedding the `gpgsig` header in the raw git object, which is not possible via its REST API.
::

### How it works

Studio passes commit payloads to a server-side signing route (`/__nuxt_studio/git/sign-commit`). The route uses your server-configured PGP private key to produce a detached ASCII-armored signature, which is then included in the `POST /git/commits` call to GitHub. GitHub re-derives the canonical commit object and verifies the signature.

Signing is best-effort. If the signing route fails because the key is missing, malformed, or protected by the wrong passphrase, Studio logs the failure and publishes the commit unsigned instead of blocking the editor's pending changes.

### Requirements

The PGP key's user-id email **must match a verified email on the GitHub account** that appears as the commit author. If the emails don't match, GitHub will show the commit as "Unverified" even though the signature is cryptographically valid.
...
```bash [.env]
# ASCII-armored PGP private key (required for signing)
STUDIO_SIGNING_PRIVATE_KEY="-----BEGIN PGP PRIVATE KEY BLOCK-----
...
-----END PGP PRIVATE KEY BLOCK-----"

# Passphrase protecting the key (optional, omit if key has no passphrase)
STUDIO_SIGNING_KEY_PASSPHRASE=your-passphrase
```

#### Enable signing in your config
...
export default defineNuxtConfig({
  studio: {
    git: {
      signing: {
        enabled: true,
        // privateKey and passphrase are read from env vars automatically
        // You can also inline them here (not recommended for secrets):
        // privateKey: process.env.STUDIO_SIGNING_PRIVATE_KEY,
        // passphrase: process.env.STUDIO_SIGNING_KEY_PASSPHRASE,
      }
    }
  }
})
...
- The private key never leaves your server — it is stored in environment variables and only accessed by the server-side signing route.
- In development mode, the signing route does not require authentication (consistent with all other Studio routes in dev mode).
- In production, the route requires a valid Studio session.
