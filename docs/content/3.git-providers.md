...
When deploying on **Vercel**, **Netlify**, **GitHub Actions**, **GitLab CI**, or **Cloudflare Workers / Pages**, Studio automatically detects repository details from environment variables and git metadata.
...
| **Cloudflare Workers / Pages** | from git remote       | from git remote          | from git remote          | `WORKERS_CI_BRANCH` / `CF_PAGES_BRANCH` |

On Vercel, Netlify, GitHub Actions, and GitLab CI you can skip `studio.repository` entirely — all fields are detected.
On Cloudflare, `branch` comes from `WORKERS_CI_BRANCH` / `CF_PAGES_BRANCH` and the rest is auto-detected from the current git remote during the build.
...
::note
Override via `STUDIO_REPO_URL=https://github.com/owner/repo`.
::
