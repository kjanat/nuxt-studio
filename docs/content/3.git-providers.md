...
When deploying on **Vercel**, **Netlify**, **GitHub Actions**, **GitLab CI**, or **Cloudflare Workers / Pages**, Studio automatically detects repository details from the platform's environment variables.
...
| **Cloudflare Workers/Pages**  | from `.git/config`    | \"                       | \"                       | `WORKERS_CI_BRANCH`/`CF_PAGES_BRANCH` |

On Vercel, Netlify, GitHub Actions, and GitLab CI you can skip `studio.repository` entirely — all fields are detected.
On Cloudflare, `branch` comes from `WORKERS_CI_BRANCH`/`CF_PAGES_BRANCH` and the rest is auto-detected from `.git/config` during the build.
